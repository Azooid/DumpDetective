...
/// <summary>
/// Result of a single shared heap walk built once via <see cref="DumpContext.EnsureSnapshot"/>
/// and reused across all sub-commands in <c>analyze --full</c>, eliminating redundant heap walks.
///
/// What is cached in one pass:
///   • Per-type statistics (count, size, per-generation breakdowns, up to 5 sample addresses)
///   • Inbound reference counts per object address  (replaces HighRefsCommand Pass 1)
///   • String value → (count, total size)           (replaces StringDuplicatesCommand heap walk)
///   • Generation byte totals and object counts     (replaces GenSummaryCommand object-count walk)
///   • Frozen / POH object detail                   (replaces GenSummaryCommand frozen detail walk)
/// </summary>
internal sealed class HeapSnapshot
{
    /// <summary>Per-type statistics, keyed by type name.</summary>
    internal Dictionary<string, TypeAgg> TypeStats { get; }

    /// <summary>Object address → inbound reference count. Replaces HighRefsCommand Pass 1.</summary>
    internal Dictionary<ulong, int> InboundCounts { get; }

    /// <summary>String value → (count, total size). Replaces StringDuplicatesCommand heap walk.</summary>
    internal Dictionary<string, (int Count, long TotalSize)> StringGroups { get; }

    // ── Generation byte totals (for MemoryLeakCommand) ────────────────────────
    ...
    // ── Generation object counts (for GenSummaryCommand.ScanObjectCounts) ─────
    internal long Gen0ObjCount { get; }
    internal long Gen1ObjCount { get; }
    internal long Gen2ObjCount { get; }

    // ── Frozen / POH object detail (for GenSummaryCommand.RenderFrozenPohDetail)
        ...
        long totalStringCount, long totalStringSize)
    {
        ...
        Gen1ObjCount     = gen1c;
        Gen2ObjCount     = gen2c;
        FrozenObjCount   = frozenObjCount;
        FrozenObjSize    = frozenObjSize;
        PohObjCount      = pohObjCount;
        PohObjSize       = pohObjSize;
        TotalObjects     = totalObjs;
        TotalRefs        = totalRefs;
        TotalStringCount = totalStringCount;
        TotalStringSize  = totalStringSize;
    }
    ...
    {
        var typeStats     = new Dictionary<string, TypeAgg>(capacity: 2048, StringComparer.Ordinal);
        ...
        foreach (var obj in ctx.Heap.EnumerateObjects())
        {
            ...
            switch (seg?.Kind)
            {
                ...
                case GCSegmentKind.Ephemeral when seg is not null:
                    ...
                    break;
                default: g2 = true; break; // unknown segment — treat as Gen2
            }
            ...
            if (!typeStats.TryGetValue(name, out var acc))
            {
                acc = new TypeAgg
                {
                    ...
                    // GenLabel is set from the first instance seen — mirrors HeapStatsCommand's
                    // mtToGen cache which uses the first object's segment kind per MethodTable.
            ...
            // ── Inbound reference counts (Pass 1 for HighRefsCommand) ─────────
            // Enumerating outgoing refs of every object builds the inbound-count map
            // for all target addresses — no second heap pass required.
            try
            {
            ...
            // ── String deduplication (for StringDuplicatesCommand) ────────────
...
/// <summary>Per-type statistics accumulated during a <see cref="HeapSnapshot"/> walk.</summary>
internal sealed class TypeAgg
{
    public string Name     = "";
    public ulong  MT;
    public long   Count,  Size;
    public long   G0c, G0s;         // Gen0 object count and byte size
    public long   G1c, G1s;         // Gen1
    public long   G2c, G2s;         // Gen2
    public long   Lc,  Ls;          // Large Object Heap
    public long   Pc,  Ps;          // Pinned Object Heap
    /// <summary>
    /// Generation label of the first instance seen — used as the Gen column in HeapStatsCommand.
    /// Mirrors the <c>mtToGen</c> cache in <c>HeapStatsCommand.ScanHeap</c>.
    /// </summary>
