...
/// <summary>
/// All heuristic thresholds used by scoring and trend analysis.
/// Load from <c>dd-thresholds.json</c> placed next to the executable
/// (or in the working directory) to customise without recompiling.
/// </summary>
public sealed class ThresholdConfig
{
    /// <summary>Thresholds for the per-dump health score (DumpCollector.GenerateFindings).</summary>
    public ScoringThresholds Scoring { get; set; } = new();

    /// <summary>Thresholds for the trend-analysis signal table and executive summary.</summary>
...
{
    // Health score (lower = worse, so warn/crit are lower bounds)
...
internal sealed partial class ThresholdConfigContext : JsonSerializerContext { }
