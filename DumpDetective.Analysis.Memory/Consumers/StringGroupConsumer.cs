...
internal sealed class StringGroupConsumer : IHeapObjectConsumer
{
    ...
    private readonly Dictionary<string, StringGroupStats>[] _stripes;
    ...
    public Dictionary<string, StringGroupStats> StringGroups { get; private set; } = [];

    public StringGroupConsumer()
    {
        _locks   = new object[StripeCount];
        _stripes = new Dictionary<string, StringGroupStats>[StripeCount];
        for (int i = 0; i < StripeCount; i++)
        {
            _locks[i]   = new object();
            _stripes[i] = new Dictionary<string, StringGroupStats>(256, StringComparer.Ordinal);
        }
    }
    ...
    public void Consume(in ClrObject obj, HeapTypeMeta meta, ClrHeap heap)
    {
        ...
        try
        {
            ...
            lock (_locks[stripe])
            {
                ...
                sg = existed ? new StringGroupStats(sg.Count + 1, sg.TotalSize + size) : new StringGroupStats(1, size);
            }
        }
        catch { }
    }
    ...
    public void OnWalkComplete()
    {
        ...
        var merged = new Dictionary<string, StringGroupStats>(total, StringComparer.Ordinal);
        for (int i = 0; i < StripeCount; i++)
        {
