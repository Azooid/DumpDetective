...
public sealed class ConsoleSink : IRenderSink
{
    ...
    {
        if (title is not null) AnsiConsole.MarkupLine($"\n[bold]{Markup.Escape(title)}[/]");
        var grid = new Grid()
    ...
    public void Alert(AlertLevel level, string title, string? detail = null, string? advice = null)
    {
        ...
        var panel = new Panel(sb.ToString()).BorderColor(color);
    ...
    public void Reference(string label, string url)
        => AnsiConsole.MarkupLine($"  [dim]📖 {Markup.Escape(label)}:[/] [link={Markup.Escape(url)}]{Markup.Escape(url)}[/]");
