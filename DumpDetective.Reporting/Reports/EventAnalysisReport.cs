...
public sealed class EventAnalysisReport
{
    public void Render(EventAnalysisData data, IRenderSink sink, int top = 20, bool showAddr = false)
    {
        ...
        // Summary count right below the chart
        if (leakGroups > 0)
            sink.Alert(AlertLevel.Warning,
                $"{totalSubs:N0} event subscription(s) found across {data.Groups.Count} event field(s) on {data.PublisherInstanceCount:N0} publisher instance(s).",
                detail: $"{leakGroups} field(s) with > 5 subscribers — likely leak(s).");
        else
