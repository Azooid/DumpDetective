...
public sealed class RenderCommand : ICommand
{
    ...
    public int Run(string[] args)
    {
        ...
        // Default output: same path as input with .html extension.
        // "--format md" (etc.) without "--output" overrides the extension.
        // Multiple -o / --format values are supported for multi-format output.
        ...
        else
        {
            ...
            if (formats.Count > 0)
                outputPaths = [..formats.Select(f => rawFile is not null
                    ? Path.ChangeExtension(rawFile, f)
                    : $"output.{f}")];
            else
                outputPaths = rawFile is not null ? [Path.ChangeExtension(rawFile, ".html")] : [];
        }
        ...
        {
            AnsiConsole.MarkupLine("[bold red]Error:[/] could not determine output path — use --output <file>.");
            return 1;
        }
    ...
    public void Render(DumpContext ctx, IRenderSink sink) =>
        sink.Alert(AlertLevel.Warning, "render operates on JSON/BIN files, not dump files — use Run() entry point.");
